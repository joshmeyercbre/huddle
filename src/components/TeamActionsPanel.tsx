...
function sortEntriesByPriority(items: ActionEntry[]): ActionEntry[] {
...
export default function TeamActionsPanel({ initial }: { initial: ActionEntry[] }) {
  const [entries, setEntries] = useState<ActionEntry[]>(initial);
  ...
  async function toggle(id: string, current: boolean) {
    ...
    if (res.ok) {
      setEntries((prev) =>
        prev.map((e) => e.item.id === id ? { ...e, item: { ...e.item, completed: !current } } : e)
      );
    }
  }
  ...
  const open = sortEntriesByPriority(entries.filter(({ item }) => !item.completed));
  const done = entries.filter(({ item }) => item.completed);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="bg-cbre-green px-5 py-3 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-white">Team Actions</h2>
      ...
      {open.length > 0 && (
        <>
          <div className="divide-y divide-gray-100">
            {visibleOpen.map(({ item, employeeName, employeeToken }) => (
              <ActionRow
                key={item.id}
                item={item}
                employeeName={employeeName}
                employeeToken={employeeToken}
                onToggle={() => toggle(item.id, item.completed)}
      ...
      {done.length > 0 && (
        <>
          ...
          <div className="divide-y divide-gray-100">
            {visibleDone.map(({ item, employeeName, employeeToken }) => (
              <ActionRow
                key={item.id}
                item={item}
                employeeName={employeeName}
...
function ActionRow({
  ...
  return (
    <div className="px-5 py-3 flex items-center gap-3">
      ...
      <a
        href={`/huddle/${employeeToken}`}
        className="shrink-0 text-xs text-gray-400 hover:text-cbre-green transition-colors"
        title="View meeting"
      >
        ↗
      </a>
