...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log("[carry-over] previous retros found:", allPreviousRetros.length, allPreviousRetros.map(r => r.id));
    ...
    console.log("[carry-over] total items across all retros:", allItems.length, "actions:", allItems.filter(i => i.isAction).length, "open actions:", allItems.filter(i => i.isAction && !i.completedAction).length);
    ...
    console.log("[carry-over] created", created.length, "items in new retro", resource.id);
  } catch (e) {
    console.error("[carry-over] failed:", e);
  }
